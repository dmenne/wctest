  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      wellPanel(
        ...
        plotOutput("diffplot"),
        HTML(
          '<ul><li>Jeder Balken repräsentiert das 95%-Konfidenzintervall einer Gruppendifferenz.</li><li>Wenn ein Konfidenzbalken den Wert 0 NICHT kreuzt, ist diese Differenz signifikant von Null verschieden mit p<0.05.</li><li>Wenn ein Balken den Wert 0  kreuzt, dann ist die Differenz für dieses Paar NICHT SIGNIFIKANT von Null verschieden. Schreiben Sie bitte nicht: "Beide Behandlungsmethoden haben die gleiche Wirkung auf ...", sondern etwa: "Mit den Daten konnte ein Unterschied zwischen den beiden Behandlungsmethoden nicht nachgewiesen werden"; in Gedanken: ein Unterschied könnnte aber duchaus bestehen.</li><li>Wenn alle Balken den Wert 0 kreuzen, wurde für keine Paarung ein signifikanter Unterschied gefunden. </li><li>p-Werte werden hier bewusst nicht ausgegeben, da die Konfidenzintervalle aussagekräftiger sind. Falls der Betreuer der Arbeit andere Meinung ist, geben Sie ihm den Artikel  <a href = "https://www.nature.com/articles/d41586-019-00857-9">in Nature</it></a> und das Editorial im <a href="https://www.tandfonline.com/doi/full/10.1080/00031305.2019.1583913">American Statistician</a>. </li><li>Eine Korrektur für multiples Testen - Stichwort: Bonferroni - wurde nicht durchgeführt; ob man eine Korrektor anbringen sollte, oder ob es dem Leser überlassen bleibt, Ergebnisse zu bewerten, ist in der statistischen Literatur <a href="https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=2&ved=2ahUKEwjl1N31zbPiAhUDUBUIHTPGBPgQFjABegQIAxAC&url=http%3A%2F%2Fwww.stat.columbia.edu%2F~gelman%2Fresearch%2Fpublished%2Fmultiple2f.pdf&usg=AOvVaw2xpJErjzGco4hwpogMvKoO">umstritten</a>. Mein Vorschlag: Verzichten Sie darauf, von mehr als drei Gruppen die Differenzen zu bilden. Oder verwenden Sie ein Mixed-Model, sodass die Korrektur auf multiples Testen implizit vorgenommen wird.<li>Um die Graphik zu speichern, verwenden Sie im Browser das Menü, das nach Rechtsklick erscheint.</li><li>In der Tabelle unten ist "estimate" die geschätzte Differenz für den Paarvergleich; "lower" und "upper" sind die 95% Konfidenzgrenzen. Für den Wilcoxon-Test ist es die "difference in location", bestimmt mit dem <a href="https://en.wikipedia.org/wiki/Hodges%E2%80%93Lehmann_estimator">Hodges-Lehman Schätzer</a>; näherungsweise ist dies die Differenz der Mediane. Für den t-Test ist es die Differenz der Mittelwerte und deren Konfidenzintervall, unter der Annahme ungleicher Varianzen.</li></ul>'
        ),
        DTOutput('table'),
        ...
        HTML(
          'Angenommen, in der ersten Zeile steht <code>b-a  2.3 1.0 4.5</code> für Muskelkraftwerte in kg. Dann könnte der Text lauten: "<i>Bei der Gruppe mit Behandlung b ist die Muskelkraft im Mittel um 2.3 kg höher als bei Behandlung a, bei einem 95% Konfidenzintervall (KI) der Differenz von 1.0 bis 4.5 kg. Der Unterschied ist signifikant auf dem 5% Niveau.</i>" Warum: weil der Bereich nicht mit 0 überlappt. Beim zweiten Mal können Sie die Kurzform "<i>KI(1.0 bis 4.5) kg</i>" verwenden und das Signifikanzniveau (95%) weglassen;  alternativ die englische Form <it>CI</it>.<br>Steht dort: <code>b-a  0.22 -1.15 1.57</code>, dann schreiben Sie: "<i>Bei der Behandlung (mit) b ist die Muskelkraft im Mittel um 0.22 kg höher als bei Behandlung (mit) a, KI(-1.15 bis 1.57) kg; der Unterschied ist nicht signifikant.</i>" Warum: weil der Bereich mit Null überlappt.'
        ),
        hr(),
        plotOutput("boxplot"),
        hr(),
