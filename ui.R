...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel("Daten: Zweispaltige Excel-Daten aus der Zwischenablage mit STRG-V hier eingeben",
      ...
      helpText("Bevor Sie Ihre eigenen Daten auswerten, erzeugen Sie bitte durch Klick auf die Schaltfläche 'Beispiel' simulierte Daten eines Experiments, und versuchen Sie, das Ergebnis zu interpretieren. Klicken Sie nochmal und interpretieren Sie das Experiment wieder. Mehrfaches Klicken simuliert, wie die Daten der gleichen (ja!!) Studie bei mehrfachen Wiederholungen herauskommen könnten. Wenn Sie nur auf Signifikanzen schielen, werden Sie entsetzt sein, wie unterschiedlich die Ergebnisse sein können. Die Breite der Intervalle, also die Vertrauensbereiche, variieren nicht so stark. Glauben Sie also nie blind an 'signifikante Ergebnisse', schauen Sie auf Vertrauensintervalle und damit auf Effektgrößen."),
      wellPanel(id = "results",
        tags$script(type = "text/javascript",
          HTML("ace.edit('data').setOptions({tabSize:12,showInvisibles:true,useSoftTabs:false});")),
        plotOutput("diffplot"),
        HTML('<ul><li>Jeder Balken repräsentiert das 95%-Konfidenzintervale einer paarweisen Gruppendifferenz.</li><li>Wenn ein Konfidenzbalken den Wert 0 NICHT kreuzt, ist diese Differenz signifikant von Null verschieden mit p<0.05.</li><li>Wenn ein Balken den Wert 0 nicht kreuzt, dann ist die Differenz für diese paar nicht signifkant von Null verschieden. Schreiben Sie bitte nicht: "Beide Behandlungsmethoden haben die gleiche Wirkung auf ...", sondern etwa: "Mit den Daten konnte ein Unterschied zwischen den beiden Behandlungsmethoden nicht nachgewiesen werden".</li><li>Wenn alle Balken den Wert 0 kreuzen, wurde für keine Paarung ein signifikanter Unterschied gefunden. </li><li>p-Werte werden hier bewusst nicht ausgegeben, da die Konfidenzintervalle aussagekräftiger sind.</li><li>Eine Korrektur für multiples Testen - Stichwort: Bonferroni - wurde nicht durchgeführt.<li>Um die Graphik zu speichern, verwenden Sie im Browser das Menü, das nach Rechtsklick erscheint.</li><li>In der Tabelle unten ist "estimate" die geschätzte Differenz für den Paarvergleich (difference in location): "lower" und "upper" sind die 95% Konfidenzgrenzen, bestimmt mit dem <a href="https://en.wikipedia.org/wiki/Hodges%E2%80%93Lehmann_estimator">Hodges-Lehman Schätzer.</a></li><li>Falls Sie die Werte dieser Tabelle in Ihre Arbeit übernehmen, reduzieren Sie bitte die Anzahl Kommastellen einheitlich; in der hier wiedergegebenen Tabelle sind es zu viele!</li></ul>'),
