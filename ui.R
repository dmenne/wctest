...
shinyUI(fluidPage(
  ...
  mainPanel("Daten: Zweispaltige Excel-Daten aus der Zwischenablage mit STRG-V hier eingeben",
      ...
      wellPanel(id = "results",
        ...
        hr(),
        HTML("In vorigen Versionen diese Programms gab es hier noch einige hübsche Histogramme der Verteilungen, um abzuschätzen, ob die Stichprobe normalverteilt ist. Ich habe diese wieder entfernt, da sie meist nicht verstanden wurden, sondern eher zum Seitenfüllen eingesetzt wurden.")
      ) # wellPanel
    )
  )
))
